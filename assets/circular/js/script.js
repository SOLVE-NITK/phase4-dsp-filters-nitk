...
function checkconditions() {
  ...
  if (novalue1 && novalue2) {
    ...
    document.getElementById("notify").innerHTML = "Provide valid x[n] sequence and click on check to continue";
    alert("Provide valid x[n] sequence and click on check to continue");
    ...
    document.getElementById("notify").innerHTML = "Provide valid h[n] sequence and click on check to continue ";
    alert("Provide valid h[n] sequence and click on check to continue");
    ...
    document.getElementById("notify").innerHTML =
      "Visualize output graph from drop down menu and check result";
      alert("Visualize output graph from drop down menu and check result");
    document.getElementById("selectGraph").disabled = false;
    optionToEnable1.removeAttribute("disabled");
    optionToEnable2.removeAttribute("disabled");
...
function myfun() {
  ...
  for (let n = 0; n < x.length; n++) {
    ...
    for (let k = 0; k < h.length; k++) {
      const xIndex = (n - k + x.length) % x.length; // Wrap-around index for x
      convolutionResult[n] += x[xIndex] * h[k];
    }
  }
  ...
  if (selectedGraph === "x") {
    ...
    hValuesResult.textContent = `h[n] sequence: ${h.join(", ")}`;
    xValuesResult.textContent = `x[n] sequence: ${x.join(", ")}`;
    yValuesResult.textContent = `Circularly convolved sequence: ${convolutionResult.join(
      ", "
    )}`;
  }
